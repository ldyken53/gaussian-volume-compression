...
IntersectGaussiansCUDA(
	...
	auto float_opts = means3D.options().dtype(torch::kFloat32);
	torch::Tensor intersections = torch::zeros({P}, float_opts);
	torch::Tensor intersection_weights = torch::zeros({P}, float_opts);
	torch::Tensor conics = torch::zeros({P * 6}, float_opts);
	torch::Device device(torch::kCUDA);
	torch::TensorOptions options(torch::kByte);
	...
	if(P != 0)
	{
		CudaRasterizer::Rasterizer::intersect_forward(
			P,
			means3D.contiguous().data<float>(),
			scales.contiguous().data_ptr<float>(),
			scale_modifier,
			...
			gaussian_bvh,
			conics.contiguous().data<float>(),
			intersections.contiguous().data<float>(),
			intersection_weights.contiguous().data<float>(),
			debug);
	}
	return std::make_tuple(intersections, intersection_weights, conics);
}
...
IntersectGaussiansBackwardCUDA(
	...
	if(P != 0)
	{  
		CudaRasterizer::Rasterizer::intersect_backward(P,
			means3D.contiguous().data<float>(),
			scales.data_ptr<float>(),
			scale_modifier,
			volume_mins,
			...
			intersections.contiguous().data<float>(),
			intersection_weights.contiguous().data<float>(),
			dL_dintersections.contiguous().data<float>(),
			dL_dintersection_weights.contiguous().data<float>(),
			dL_dmeans3D.contiguous().data<float>(),
			dL_dscales.contiguous().data<float>(),
			dL_drotations.contiguous().data<float>(),
			debug);
